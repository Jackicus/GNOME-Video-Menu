...
#
#   ./scripts/nested.sh start [WxH]   start a nested shell (default headless 1920x1080)
#   ./scripts/nested.sh start --windowed [WxH]
#                                     start it in a visible window instead
...
#   ./scripts/nested.sh key KEYSYM    press a key (Escape, Return, Left, ...)
...
cmd_start() {
    ...
    local mode_args=(--wayland --wayland-display "$WL_DISPLAY" --virtual-monitor "$geometry")
    [[ $windowed -eq 0 ]] && mode_args+=(--headless)

    info "Starting nested GNOME Shell ($([[ $windowed -eq 1 ]] && echo windowed || echo headless), $geometry)..."
...
cmd_status() {
    if is_running; then
        echo "nested:   running (pid $(cat "$PID_FILE"))"
